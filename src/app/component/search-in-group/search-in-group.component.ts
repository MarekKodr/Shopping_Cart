...
export class SearchInGroupComponent implements OnInit {
  ...
  public addFriend(idfriend: string, email: string, foto: any): void {
    ...
    this.db.list('groups/' + this.idpar + '/lists', { preserveSnapshot: true }).subscribe(newList => {
      newList.forEach(newList => {
        this.db.object('users/' + idfriend + '/lists/' + newList.key).set({ name: newList.name });
        this.db.object('lists/' + newList.key + '/users/' + idfriend).set({ name: newList.name });
      })
    });
  }
